...
function get_temp_dependence_Z(;is_at::Bool = true)
    ...
    color2 = is_at ? :steelblue3 : :goldenrod3
    ...
    lines!(ax1, T_range, Z2, linewidth=4, linestyle = :dash, label = L"\text{model DFT}", color = color2)
    if is_at==false
...
function get_other_ther_funcs(;is_at::Bool = true)
    ...
    ax4 = Axis(fig[2,2], xlabel = L"$T$ (K)", ylabel = L"$S$\; ($10^{-3}$ a.u.)",
    ...
    lines!(ax4, T_range, S*1e03, linewidth=7, color = color1)
    ...
    lines!(ax4, T_range2, S2*1e03, linestyle = :dash, linewidth=4, color = color2)
    ...
    lines!(ax1, T_range, Z2, linewidth=4, linestyle = :dash, color = color2, label = L"\text{model DFT}")
