...
## TODO
:soon: Redo the t=0 calculations in the harmonic model (the wavefunctions overlap below the barrier - problem);
:soon: Calculate the probability density for each state in each form (true sim and harmonic model + hermite)
:soon: Calculate the differences from energy levels in the harmonic model and true sim energy levels
...
:soon: $\omega(t) = \omega + A\sin(\Omega t)$, which means $\hbar \Omega = k_{B}T$, thus $\Omega = \frac{k_B T}{\hbar}$ :arrow_right: how is $\Omega$ translate to thermal energy?
...
:grey_question: Is it worth it to calculate the expansion for the harmonic model (with Hermite)?
